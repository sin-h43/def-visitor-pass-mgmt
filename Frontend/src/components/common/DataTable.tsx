...
export default function DataTable<T>({ title, data, columns, tabs }: DataTableProps<T>) {
  ...
  return (
    <div className="bg-white border border-slate-400/80 rounded-xl overflow-hidden shadow-sm">
      ...
      <div className="p-4 border-b border-slate-400/60 bg-white flex flex-col gap-3">
        <h3 className="font-bold text-slate-800 text-lg">{title}</h3>
        {/* Right Side: Search & Filters */}
          <div className="flex justify-between items-center space-x-3">
            <div className="relative">
              <Search className="w-8 h-4 absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-400" />
              <input 
                type="text" 
                placeholder="Search visitor, host or ID..." 
                className="pl-9 pr-4 py-2 text-sm border border-slate-400/60 rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-500 w-92"
                value={searchTerm}
