...
export default function EmployeeRepeatedVisitorLogPage() {
  ...
  const columns: TableColumn<VisitorProfile>[] = [
    { key: 'name', label: 'IDENTITY PROFILE', render: (row) => ( <div className="flex items-center space-x-3"> <div className="w-8 h-8 rounded-full bg-blue-50 border border-blue-100 flex items-center justify-center text-blue-600 font-bold text-xs"> {row.name.substring(0, 2).toUpperCase()} </div> <div> <div className="font-bold text-slate-800 text-sm">{row.name}</div> <div className="text-xs text-slate-500 font-mono">{row.phone}</div> </div> </div> ) },
    ...
    { key: 'totalVisits', label: 'HISTORY VOLUME', render: (row) => ( <span className={`px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-full border ${ row.totalVisits > 1 ? 'bg-blue-50 text-blue-700 border-blue-200' : 'bg-slate-50 text-slate-600 border-slate-200' }`}> {row.totalVisits} {row.totalVisits === 1 ? 'Visit' : 'Visits'} </span> ) },
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
          ...
          <div className="flex border-b border-slate-200 text-xs font-semibold space-x-4">
            {['All Visitors', 'Frequent (2+ Visits)'].map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)} className={`pb-2 px-1 relative ${activeTab === tab ? 'text-blue-600 font-bold border-b-2 border-blue-600' : 'text-slate-400 hover:text-slate-600'}`}> {tab} </button>
            ))}
          </div>
          ...
          <div className="fixed inset-0 z-50 overflow-hidden">
            ...
            <div className="absolute inset-y-0 right-0 max-w-md w-full bg-white shadow-2xl flex flex-col animate-slide-in-right">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50/50">
                <section>
                  <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-slate-200 pb-1.5 mb-3 flex items-center"><Shield className="w-3.5 h-3.5 mr-1.5 text-blue-600" /> Authorized Profile</h3>
                  <div className="bg-white p-4 border border-slate-200 rounded-xl shadow-sm">
                    <div className="flex items-center space-x-3 mb-4 border-b border-slate-100 pb-3">
                      <div className="w-10 h-12 rounded-xl bg-blue-50 border border-blue-100 flex items-center justify-center text-blue-600 font-black text-sm">{selectedProfile.name.substring(0, 2).toUpperCase()}</div>
                ...
                <section>
                  <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-slate-200 pb-1.5 mb-3 flex items-center justify-between">
                    <span className="flex items-center"><Calendar className="w-3.5 h-3.5 mr-1.5 text-blue-600" /> My Logged Timeline</span>
                    <span className="text-[9px] bg-blue-100 text-blue-800 px-2 py-0.5 rounded font-bold uppercase tracking-wider">{selectedProfile.totalVisits} Session Passes</span>
                  </h3>
